...
4. ~~**Muted text opacity divergence**~~ (shipped 2026-03-18) -- Standardized to `text-muted` on light, `text-white/60` on dark.
...
10. **CTA section layout variety audit** -- Verify CTA button styling is identical everywhere even though section layouts vary.
11. **`getBtnCenter()` naming debt** -- Function uses wrap rect but is named for button (noted in debrief).
12. **Remaining opacity-30/opacity-40 standardization** -- Calendar day headers, time labels, and confirmation text still use bare opacity classes. Could be migrated to `text-white/30` and `text-white/40` for consistency, but lower priority since these are intentionally very faint UI chrome, not muted body text.
